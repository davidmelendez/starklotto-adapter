...
export const RandomnessComponent = ({
  ...
  const isDevnet = chain?.network === "devnet" || targetNetwork.network === "devnet";
  ...
  const handleRequestRandomness = async () => {
    ...
    if (account?.address === "0x0297fd6c19289a017d50b1b65a07ea4db27596a8fade85c6b9622a3f9a24d2a9") {
      console.warn("🚨 CUENTA PROBLEMÁTICA DETECTADA:", account.address);
      notification.error("Se ha detectado una cuenta que puede causar problemas. Intenta reconectar tu wallet o usar una cuenta diferente.");
      return;
    }
    ...
    console.log("🔍 DIAGNÓSTICO - Información del contrato:", {
      contractName,
      contractAddress,
      expectedAddress: "0x31cdafdd0fc1a80d57f3290afff3ba0a62e9d2c628e35c81eb55e05879f0f4f",
      ...
      isDevnet: chain?.network === "devnet" || targetNetwork.network === "devnet",
    });
    ...
    console.log("🔍 DIAGNÓSTICO - Información de la cuenta y transacción:", {
      ...
      writeDisabledReason: writeDisabled ? "Wallet en red incorrecta o desconectada" : "Listo para transacción",
      contractAddress,
      functionToCall: isDevnet ? "devnet_generate" : "request_randomness_prod",
      calldataParams: isDevnet ? ["seed"] : ["seed", "callbackFeeLimit", "publishDelay"],
    });
    ...
    try {
      ...
      const isDevnet = forceDevMode || chain?.network === "devnet" || targetNetwork.network === "devnet";

      console.log("🎯 Modo detectado:", isDevnet ? "DESARROLLO" : "PRODUCCIÓN", forceDevMode ? "(FORZADO)" : "");
      ...
      if (isDevnet) {
        ...
        if (txHash) {
          setTxHash(txHash);
          console.log("✅ Generación de desarrollo ejecutada exitosamente", { transactionHash: txHash });
          notification.success(`¡5 números aleatorios generados exitosamente! Hash: ${txHash}`);
          if (onSuccess) {
        ...
        if (useAlternativeMode) {
          ...
          console.log("🔧 Ejecutando en modo ALTERNATIVO con MULTICALL (parámetros seguros)", {
            contractAddress,
            seed: seedValue.toString(),
            account: account?.address,
          ...
          if (multicallTx) {
            ...
            console.log("✅ Multicall alternativo ejecutado exitosamente", { transactionHash: multicallTx });
            notification.success(`¡Solicitud VRF enviada (Modo Seguro)! Hash: ${multicallTx}. Esperando respuesta del oráculo...`);
            if (onSuccess) {
          ...
          console.log("🏭 Ejecutando en modo PRODUCCIÓN con MULTICALL (estándar)", {
            contractAddress,
            seed: seedValue.toString(),
            callbackFeeLimit,
            publishDelay,
          ...
          if (multicallTx) {
            setTxHash(multicallTx);
            console.log("✅ Multicall estándar ejecutado exitosamente", { transactionHash: multicallTx });
            notification.success(`¡Solicitud VRF enviada! Hash: ${multicallTx}. Esperando respuesta del oráculo...`);
            if (onSuccess) {
              onSuccess(multicallTx, generationId);
            }
          }
    ...
    <div className="space-y-6 p-6 bg-component border border-[#8A45FC] rounded-[5px]">
      <div className="space-y-4">
        ...
        <p className="text-sm text-gray-300">
          {isDevnet ? (
            <>Esta función genera 5 números aleatorios únicos en el rango [1,49] usando generación local para desarrollo.</>
          ) : (
            <>Esta función ejecuta un multicall que primero solicita aleatoriedad al VRF provider de Cartridge, luego consume esa aleatoriedad para generar 5 números únicos en el rango [1,49].</>
          )}
        </p>
        ...
        <div className="bg-base-100 p-4 rounded-lg">
          ...
          <div className="mt-2 text-xs text-gray-400">
            <p><strong>Dirección esperada:</strong> 0x31cdafdd0fc1a80d57f3290afff3ba0a62e9d2c628e35c81eb55e05879f0f4f</p>
            <p><strong>Dirección actual:</strong> {contractAddress}</p>
            <p><strong>Red:</strong> {chain?.name || "Desconocida"} → {targetNetwork.name}</p>
            <p><strong>Modo:</strong> {isDevnet ? "Desarrollo (devnet)" : "Producción (testnet/sepolia)"}</p>
          </div>
        </div>

        {/* Información técnica */}
        <div className={`${isDevnet ? 'bg-blue-900/20 border-blue-600' : forceDevMode ? 'bg-yellow-900/20 border-yellow-600' : 'bg-purple-900/20 border-purple-600'} p-4 rounded-lg`}>
          <h4 className={`font-semibold mb-2 ${isDevnet ? 'text-blue-300' : forceDevMode ? 'text-yellow-300' : 'text-purple-300'}`}>
          ...
          <div className="space-y-1 text-sm">
            {isDevnet ? (
              <>
                <p><strong>Método:</strong> devnet_generate (generación local)</p>
                <p><strong>Contrato:</strong> {contractAddress}</p>
                <p><strong>Estado:</strong> {forceDevMode ? 'Forzado para testing' : 'Automático'}</p>
              </>
            ) : (
              <>
                <p><strong>Método:</strong> Multicall VRF ({useAlternativeMode ? 'Modo Seguro' : 'Estándar'})</p>
                <p><strong>Transacción 1:</strong> request_random → VRF Provider</p>
                <p><strong>Transacción 2:</strong> request_randomness_prod → Contrato</p>
                <p><strong>VRF Provider:</strong> {VRF_PROVIDER_ADDRESS}</p>
                <p><strong>Callback Fee Limit:</strong> {useAlternativeMode ? '50,000' : callbackFeeLimit} wei</p>
                <p><strong>Publish Delay:</strong> {publishDelay} (sin delay)</p>
                <p><strong>Source (Seed):</strong> Usado como source para VRF</p>
              </>
            )}
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          {contractAddress && contractAddress !== "0x31cdafdd0fc1a80d57f3290afff3ba0a62e9d2c628e35c81eb55e05879f0f4f" && (
            <div className="bg-red-900/20 p-3 rounded-lg border border-red-600">
              <h4 className="font-semibold text-red-300 mb-2">🚨 Problema Detectado</h4>
              <div className="text-sm space-y-1 text-red-200">
                <p><strong>Dirección del contrato incorrecta:</strong></p>
                <p>• Dirección esperada: 0x31cdafdd0fc1a80d57f3290afff3ba0a62e9d2c628e35c81eb55e05879f0f4f</p>
                <p>• Dirección actual: {contractAddress}</p>
                <p>• <strong>Solución:</strong> El contrato necesita ser recompilado y redeployado con la dirección correcta.</p>
              </div>
            </div>
          )}

          {/* Diagnóstico específico de problemas de cuenta/wallet */}
          {account?.address && account.address.startsWith("0x0297fd6c19289a017d50b1b65a07ea4db27596a8fade85c6b9622a3f9a24d2a9") && (
            <div className="bg-red-900/20 p-3 rounded-lg border border-red-600">
              <h4 className="font-semibold text-red-300 mb-2">🚨 Cuenta Problemática Detectada</h4>
              <div className="text-sm space-y-2 text-red-200">
                <p><strong>Se ha detectado una cuenta que causa errores de transacción.</strong></p>
                <div className="bg-red-800/30 p-2 rounded text-xs">
                  <p><strong>Dirección problemática:</strong></p>
                  <p className="font-mono break-all">{account.address}</p>
                </div>

                <div className="space-y-2">
                  <p className="font-semibold">🔧 Opciones para solucionar:</p>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-2">
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setSeed("12345");
                        notification.info("Modo Seguro activado. Intenta generar números con parámetros más conservadores.");
                      }}
                    >
                      🛡️ Modo Seguro
                    </button>
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setSeed(newSeed);
                        notification.info(`Nueva semilla generada: ${newSeed}. Intenta generar números con esta semilla diferente.`);
                      }}
                    >
                      🎲 Nueva Semilla
                    </button>
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        notification.info(forceDevMode ? "Modo desarrollo desactivado." : "Modo desarrollo forzado activado.");
                      }}
                    >
          ...
          {forceDevMode && (
            <div className="bg-yellow-900/20 p-3 rounded-lg border border-yellow-600">
              <h4 className="font-semibold text-yellow-300 mb-2">⚠️ Modo Desarrollo Forzado</h4>
              <div className="text-sm space-y-1 text-yellow-200">
                <p>• Usando función de desarrollo (devnet_generate) incluso en testnet</p>
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    notification.info("Modo desarrollo desactivado. Usando modo producción.");
                  }}
                >
                  ❌ Desactivar Modo Dev
                </button>
        ...
        <div className="bg-blue-900/20 p-4 rounded-lg border border-blue-600">
          ...
          {isDevnet ? (
            <ol className="text-sm space-y-1 text-gray-300">
              <li>1. Se llama directamente a <code>devnet_generate(seed)</code></li>
              <li>2. El contrato genera 5 números únicos usando un algoritmo LCG local</li>
              <li>3. Los números se generan inmediatamente sin depender de oráculos externos</li>
              <li>4. Los números se almacenan y se pueden consultar con <code>get_generation_numbers(id)</code></li>
            </ol>
          ) : (
            <ol className="text-sm space-y-1 text-gray-300">
              <li>1. <strong>Paso 1:</strong> Se ejecuta multicall con 2 transacciones</li>
              <li>2. <strong>Transacción 1:</strong> <code>request_random(caller, source)</code> → VRF Provider</li>
              <li>3. <strong>Transacción 2:</strong> <code>request_randomness_prod(seed, fee, delay)</code> → Contrato</li>
              <li>4. El contrato solicita y consume aleatoriedad usando protocolo VRF de Cartridge</li>
              <li>5. Los números se generan usando aleatoriedad descentralizada verificable</li>
              <li>6. Los números se almacenan y se pueden consultar con <code>get_generation_numbers(id)</code></li>
            </ol>
          )}
        </div>
      </div>
...
const VRFCoordinatorConfig = ({ contractAddress }: VRFCoordinatorConfigProps) => {
  const [newCoordinatorAddress, setNewCoordinatorAddress] = useState<string>(VRF_PROVIDER_ADDRESS);
  const [isLoading, setIsLoading] = useState(false);
  ...
  const handleUpdateCoordinator = async () => {
    if (!newCoordinatorAddress || !newCoordinatorAddress.startsWith('0x') || newCoordinatorAddress.length !== 66) {
    ...
    try {
      ...
      if (txHash) {
        notification.success(`VRF coordinator actualizado exitosamente! Hash: ${txHash}`);
        setIsExpanded(false);
      }
    } catch (error: any) {
      console.error("Error actualizando VRF coordinator:", error);
      notification.error("Error actualizando VRF coordinator: " + (error.message || "Error desconocido"));
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            ...
            <div className="text-xs text-gray-400 space-y-1">
              <p><strong>Dirección actual configurada:</strong> {VRF_PROVIDER_ADDRESS}</p>
              <p><strong>Dirección en formulario:</strong> {newCoordinatorAddress}</p>
              <p><em>Nota: Solo el owner del contrato puede cambiar esta configuración.</em></p>
            </div>

            <button
              onClick={handleUpdateCoordinator}
              disabled={isLoading || newCoordinatorAddress === VRF_PROVIDER_ADDRESS}
          ...
          <details className="text-sm">
            ...
            <div className="mt-2 space-y-1 text-gray-400 text-xs">
              <p>• Esta función llama a <code>set_vrf_coordinator()</code> en el contrato</p>
              <p>• Solo el owner del contrato puede ejecutar esta función</p>
              <p>• El contrato usará esta dirección para validar callbacks del VRF</p>
              <p>• Asegúrate de usar la dirección correcta del VRF provider de Cartridge</p>
            </div>
          </details>
        </div>
      )}
